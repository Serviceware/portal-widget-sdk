 ...
 * CSS compliance probe. Translations come from build.mjs's mock API.
 */
...
function makeConfig(customConfig: Record<string, unknown> = {}): WidgetConfig {
    return {
        ...
        customConfig,
        customTrans: {},
        meta: [{ language: 'en', name: 'Serviceware Logo 3D' }],
        usedVersion: {
            id: 'version-1',
            name: TAG,
            number: VERSION,
        ...
        // A linked service connection as the Portal delivers it today, header value included
        // (docs/backend-findings.md section 2). The widget must only ever show its name.
        serviceConnectionsId: ['sc-1'],
        serviceConnections: [
            {
                id: 'sc-1',
                ...
                isDefault: true,
                security: { mode: 2, headers: [{ key: 'x-api-key', value: 'fake-dev-key-never-real' }] }
            }
        ]
    };
}
...
/**
 * Captures the widget's WebGL canvas in the same animation frame it renders (the drawing buffer is not
 * preserved between frames) and posts it to the dev server, which writes dev/last-frame.png.
 */
function saveFrame(): void {
    const canvas = mainElement?.querySelector('canvas') ?? mainElement?.shadowRoot?.querySelector('canvas');
    if (!canvas) {
        log('save frame', 'no canvas found', true);
        return;
    }
    requestAnimationFrame(() =>
        requestAnimationFrame(() => {
            ...
            context.fillStyle = '#e4ebf2';
            context.fillRect(0, 0, copy.width, copy.height);
            context.drawImage(canvas, 0, 0);
            copy.toBlob(blob => {
                if (!blob) {
                    log('save frame', 'canvas export failed', true);
                    return;
                }
                fetch('/dev/frame', { method: 'POST', body: blob })
                    .then(response => response.json())
                    .then(result => log('save frame', result))
                    .catch(error => log('save frame', String(error), true));
            }, 'image/png');
        })
    );
}
byId<HTMLButtonElement>('frame').addEventListener('click', saveFrame);
