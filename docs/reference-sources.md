...
| Fact | Source |
|---|---|
| Window events are `CustomEvent`s on `window`, type = name + `SSPEvent`, payload in `detail`, `handled` flag | `libs/ssp-core/src/event-manager.ts` (snapshot in `reference/ssp-portal/`) |
...
| Element inputs and outputs | `libs/common/src/lib/page/containers/widget/widget.component.html` (snapshot) |
...
| Bearer only for `apiUrl` prefix | `libs/utils/src/lib/services/auth-interceptor/auth-interceptor.ts` (snapshot) |
...
| Globals exposed for light builds | `libs/common/src/shared-dependencies-enum.ts`, `libs/common/src/expose-global-modules.ts`, `libs/utils/src/shared/widgets/webpack.externals.js` (snapshot) |
| Compliance probe rules | `libs/common/src/lib/page/containers/widget/utils/widget-compliance.ts` (snapshot) |
...
| CSS scoping pipeline | `libs/utils/src/shared/postcss-widget-scope-plugin.js`, `build-styles.js`, `.scripts/rename-widget.js` (snapshots) |
...
| Zip format | `libs/admin/src/lib/widgets-management/services/widget-package/widget-package.service.ts` (snapshot) |
...
| Release tool behaviour | `libs/utils/src/shared/tools/prepare-widget-release.ts` (snapshot), `libs/utils/docs/PREPARE_WIDGET_RELEASE.md` |
